(function(impress) {
	...
	impress.server.start = function() {
                ...
                impress.conn.on('query', function(err, results, fields, query) {
                    console.dir({SQL:query.sql});
                });
                //svr93// end
	...
	//
	Client.prototype.setCookie = function(name, value, host) {
		...
		client.fileHandler('access', function() {
			if (client.access.index) {
				...
				impress.fs.readdir(indexPath, function(err, flist) {
					...
					dirs.push({name: '/..', path: '..', size: 'up', mtime:' '});
					for (var i in flist) {
						(function() {
							...
							impress.fs.stat(filePath, function(err, stats) {
								if (!err) {
									...
									if (stats.isDirectory()) dirs.push({name:'/'+fileName, path:fileName+'/', size:'dir', mtime:mtime});
									else files.push({name:fileName, path:fileName, size:bytesToSize(stats.size).replace(/\s/, '&nbsp;'), mtime:mtime});
								}
								if (++cbIndex>=cbCount) {
									files = dirs.concat(files);
									client.include({title:"Directory index", path:client.url, files:files}, impress.templatesDir+'/index.template', '', function(tpl) {
										client.end(tpl);
									});
								};
							});
